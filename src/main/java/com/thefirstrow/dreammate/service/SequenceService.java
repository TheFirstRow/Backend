...
public class SequenceService {
    ...
    // 시퀀스 조회
    public List<SequenceEntity> getMySequences(Long userId) {
    ...
    // 시퀀스 수정
    @Transactional
    public SequenceEntity updateSequence(Long userId, Long sequenceId, String stageTitle, String stageContent, Long musicMs, String targetObject, Long effectNumber, String cameraName, Float x, Float y, Float z) {
        ...
        if (!Objects.equals(sequenceEntity.getUser().getId(), userId)) {
            throw new DreamMateApplicationException(ErrorCode.INVALID_PERMISSION, String.format("user %s has no permission with sequence %d", userId, sequenceId));
        }
